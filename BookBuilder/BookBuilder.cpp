...
void bookBuilder(SPSCQueue<BookBuilderGatewayToComponentQueueEntry>& bookBuilderGatewayToComponentQueue, SPSCQueue<OrderBook>& bookBuilderToStrategyQueue, std::vector<std::string> currencyPairs) {
    ...
    long exchangeUpdateTxTimestamp;
    GenericValue<rapidjson::UTF8<>>::MemberIterator data;
#if defined(USE_BITMEX_EXCHANGE) || defined(USE_BITMEX_MOCK_EXCHANGE)
    const char *action, *symbol, *side, *timestamp;
    uint64_t id;
    double size, price;
#elif defined(USE_KRAKEN_EXCHANGE) || defined(USE_KRAKEN_MOCK_EXCHANGE)
    const char *type, *symbol, *timestamp;
    double price, size;
    uint64_t checksum;
    ...
    while (true) {
        ...
        while (currentPos < queueEntry.decryptedReadBuffer + strlen(queueEntry.decryptedReadBuffer)) {
            ...
            for (SizeType i = 0; i < doc["data"].Size(); i++) {
                ...
                timestamp = data_i["timestamp"].GetString();
                exchangeUpdateTxTimestamp = convertTimestampToTimePoint(timestamp);
                ...
                if (side[0] == 'B') {
                    switch (action[0]) {
                        case 'p':
                        case 'i':
                            orderBookMap[symbol].insertBuy(id, price, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadFinishTimestamp);
                            break;
                        case 'u':
                            orderBookMap[symbol].updateBuy(id, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadFinishTimestamp);
                            break;
                        case 'd':
                            orderBookMap[symbol].removeBuy(id, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadFinishTimestamp);
                            break;
                        default:
                            break;
                    }
                    ...
                    switch (action[0]) {
                        case 'p':
                        case 'i':
                            orderBookMap[symbol].insertSell(id, price, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadFinishTimestamp);
                            break;
                        case 'u':
                            orderBookMap[symbol].updateSell(id, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadFinishTimestamp);
                            break;
                        case 'd':
                            orderBookMap[symbol].removeSell(id, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadFinishTimestamp);
                            break;
                        default:
                            break;
                    }
            ...
            for (SizeType i = 0; i < doc["data"].Size(); i++) {
                ...
                if (type[0] == 's') {
                    ...
                    for (SizeType i = 0; i < data_i["asks"].Size(); i++) {
                        ...
                        size = ask_i["qty"].GetDouble();
                        if (size == 0)
                            orderBookMap[symbol].removeSell(price, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadCompletionTimestamp);
                        else if (orderBookMap[symbol].checkSellSidePriceLevel(price))
                            orderBookMap[symbol].updateSell(price, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadCompletionTimestamp);
                        else 
                            orderBookMap[symbol].insertSell(price, price, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadCompletionTimestamp);
                    }
                    for (SizeType i = 0; i < data_i["bids"].Size(); i++) {
                        const Value& bid_i = bids->value[i]; 
                        price = bid_i["price"].GetDouble();
                        size = bid_i["qty"].GetDouble();
                        if (size == 0)
                            orderBookMap[symbol].removeBuy(price, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadCompletionTimestamp);
                        else if (orderBookMap[symbol].checkBuySidePriceLevel(price))
                            orderBookMap[symbol].updateBuy(price, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadCompletionTimestamp);
                        else 
                            orderBookMap[symbol].insertBuy(price, price, size, exchangeUpdateTxTimestamp, queueEntry.marketUpdateReadCompletionTimestamp);
                    }
                }
                while (!bookBuilderToStrategyQueue.push(orderBookMap[symbol]));    
            ...
            double networkLatency = (socketRxUs - exchangeUpdateTxTimestamp) / 1000.0; 
